...
# %%
def eval_gp(model,x_test,noises_test,likelihood,x_scaler=[],inputScaled=True,y_scaler=[],outputScaled=False):
...
#         y_scaler, x_scaler (sklearn.preprocessing.StandardScaler(), required if in/outputs are to be scaled)
    ...
    model.eval()
    with torch.no_grad():
        trained_pred_dist = likelihood(model(x_test),noises=noises_test)
