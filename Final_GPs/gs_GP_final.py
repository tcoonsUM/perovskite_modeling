...
print("import data")
df = pd.read_csv('Final_GPs/20231027_grainsize_inputs.csv')  
# Uncomment the line below to get a fresh split!
#df = df.sample(frac=1)
x = df.iloc[:,1:4]
y = df.loc[:,"Estimated Grain Size (nm)mean"]
...
df_results.to_csv('./heatmaps/GS_GP_heatmap.csv', index=False)
#%%
levelsi=np.linspace(100,600,num=100)
for test_time in [5,10,15]:
    ...
    x=np.array(x)
    y=np.array(y)
    mi = 100#min(lower_inverse[li:ui,:])[0]#np.min((min(lower_inverse[li:ui,:]), min(trueYs)))
    ma = 600#max(lower_inverse[li:ui,:])[0]#np.max((max(lower_inverse[li:ui,:]), max(trueYs)))
