...
y_upper = y+2*noises; y_lower = y-2*noises

# defining priors
prior_bounds = np.array([5, 99])
...
for i in range(nData): # i indicates the data index to leave out as a test dataset
    ...
    y_train = y_scaler.transform(y_train.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_lower_train = y_scaler.transform(y_lower_train.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_upper_train = y_scaler.transform(y_upper_train.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_test = y_scaler.transform(y_test.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_lower_test = y_scaler.transform(y_lower_test.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_upper_test = y_scaler.transform(y_upper_test.reshape(-1,1)) * (1-1e-8) + 1e-9
    y_bounds = y_scaler.transform(prior_bounds.reshape(-1,1)) * (1-1e-8) + 1e-9
    ...
    noises_train = (y_upper_train - y_lower_train)/4
    noises_test = (y_upper_test - y_lower_test)/4
    y_prior_std = (y_bounds[1]-y_bounds[0])/4
...
# scale y data to U(0,1)
y_scaler = preprocessing.QuantileTransformer(n_quantiles=nData).fit(y_train.reshape(-1,1))
print(y_train[2])
y_train = y_scaler.transform(y_train.reshape(-1,1)) 
print(y_train[2])
y_train = y_train* 0.99999 + 1e-16
print(y_train[2])
y_lower_train = y_scaler.transform(y_lower_train.reshape(-1,1)) * 0.99999 + 1e-11
y_upper_train = y_scaler.transform(y_upper_train.reshape(-1,1)) * 0.99999 + 1e-11
y_bounds = y_scaler.transform(prior_bounds.reshape(-1,1)) * 0.99999 + 1e-11
...
upper_inverse = norm.cdf(upper)
predictive_mean_inverse = norm.cdf(predictive_mean)
predictive_mean_inverse = np.array(y_scaler.inverse_transform(predictive_mean_inverse.reshape(-1,1)))
upper_inverse = np.array(y_scaler.inverse_transform(upper_inverse.reshape(-1,1)))
lower_inverse = np.array(y_scaler.inverse_transform(lower_inverse.reshape(-1,1)))
